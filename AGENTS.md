...
> **Read this first.** This file is the single source of truth for any AI model working on gonorth.co.il. Whether you are Codex, GPT-4, Gemini, or another model — start here before doing anything.
...
Agents:     read .Codex/agents/{agent-name}.md for autonomous agent definitions
```
...
ssh -i ~/.ssh/gonorth_rsa -o StrictHostKeyChecking=no Codex@204.168.145.116
...
Agents are autonomous AI assistants defined in `.Codex/agents/`. Each has a specific role, model, and set of skills.
...
| Category (AGENTS.md) | Keyword section (SEO-KEYWORDS.md) |
|---|---|
